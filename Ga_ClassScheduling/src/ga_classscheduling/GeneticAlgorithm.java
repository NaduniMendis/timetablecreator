...
public class GeneticAlgorithm {
	...
	Population crossoverPopulation(Population population) {
		...
		IntStream.range(0, Driver.NUMB_OF_ELITE_SCHDEULES).forEach(x -> crossoverPopulation.getSchedules().set(x, population.getSchedules().get(x)));
		IntStream.range(Driver.NUMB_OF_ELITE_SCHDEULES, population.getSchedules().size()).forEach(x -> {
	...
	Population mutatePopulation(Population population) {
		...
		IntStream.range(0, Driver.NUMB_OF_ELITE_SCHDEULES)
