...
public class Schedule {
	...
	public Schedule initialize() {
		...
		new ArrayList<Department>(data.getDepartment()).forEach(dept -> {
			dept.getCourses().forEach(course -> {
				...
				// newClass.setMeetingTime(data.getMeetingTimes().get((int)
				// (data.getMeetingTimes().size()*Math.random())));
	...
	private double calculateFitness() {
		numbOfConflicts = 0;
		classes.forEach(x -> {
			// if (x.getRoom().getSeatingCapacity()
			// <x.getCourse().getMaxNumbOfStudents()) numbOfConflicts++;
			if (x.getRoom().getSeatingCapacity() < x.getCourse().maxNumbOfStudents())
				numbOfConflicts++;
